...
// Импортируем библиотеку Tippy.js и её базовые стили
import tippy from 'tippy.js';
import 'tippy.js/dist/tippy.css';

/**
 * Инициализирует и настраивает тултип для указанного элемента.
 * @param {HTMLElement} element - Элемент-триггер, на который вешается тултип.
 * @param {object} promo - Объект с данными акции.
 */
export function setupTooltipEvents(element, promo) {
  // Вызываем Tippy.js для нашего элемента
  tippy(element, {
    // Разрешаем HTML-контент внутри тултипа
    allowHTML: true,
    // Делаем тултип интерактивным (можно навести мышь)
    interactive: true,
    // Позиционируем снизу с выравниванием по правому краю
    placement: 'bottom-end',
    // Применяем нашу кастомную тему из promo-cards.scss
    theme: 'promo-theme',
    
    // Эта функция сработает один раз перед первым показом тултипа
    onShow(instance) {
      // Создаем HTML-элемент для контента
      ...
      // Находим кнопку "копировать" и вешаем на нее обработчик
      const copyButton = content.querySelector('.tooltip-copy-button');
      copyButton.addEventListener('click', () => {
         navigator.clipboard.writeText(promo.erid);
         instance.setContent('Скопировано!'); // Меняем текст тултипа
         setTimeout(() => {
            instance.hide(); // Скрываем тултип через 1.5 секунды
         }, 1500);
      });
      
      // Устанавливаем наш HTML как контент для этого экземпляра Tippy
